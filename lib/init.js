...
const REQUIRED_SKILL_FILES = [
  ...
  path.join('scripts', 'harness_v2', 'adapters', 'hooks', 'phase_guard_v2.py'),
  path.join('scripts', 'harness_v2', 'adapters', 'hooks', 'stop_guard_v2.py'),
];
...
// v2 ships only the claude settings.json hook template, so init always targets
...
function detectRuntime({ projectRoot, runtime }) {
  ...
  if (has('.codex')) {
    return {
      runtime: 'claude',
      warning: 'project has .codex but no .claude; the v2 CLI installs claude-format hooks into .claude/settings.json. For OpenCode, wire the opencode-plugin example instead.',
    };
  }
