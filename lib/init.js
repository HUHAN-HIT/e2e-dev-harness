...
const opencodeHooks = require('./opencode-hooks');
...
function detectRuntime({ projectRoot, runtime }) {
  // opencode enforces the code-write phase lock but cannot enforce run-to-VERIFIED:
  // it has no Stop-veto hook (session.idle is observe-only), so the stop guard is
  // advisory there. Surface that wherever the opencode runtime resolves.
  const OPENCODE_WARNING = 'opencode runtime: the code-write phase lock is enforced, but the '
    + 'run-to-VERIFIED stop guard is advisory only (opencode has no stop-veto hook).';
  if (runtime && runtime !== 'auto') {
    return { runtime, warning: runtime === 'opencode' ? OPENCODE_WARNING : null };
  }
  const has = (dir) => fs.existsSync(path.join(projectRoot, dir));
  if (has('.claude')) return { runtime: 'claude', warning: null };
  if (has('.opencode')) return { runtime: 'opencode', warning: OPENCODE_WARNING };
  if (has('.codex')) {
    return {
      runtime: 'claude',
...
function runInit({ projectRoot, runtime = 'auto', dryRun = false, doctor = true, force = false, deps = {} }) {
  const d = {
    ...
    materializeOpencodePlugin: opencodeHooks.materializeOpencodePlugin,
  ...
  const hookResult = resolvedRuntime === 'opencode'
    ? d.materializeOpencodePlugin({ skillHome: skill.home, projectRoot, dryRun })
    : d.materializeHooks({ skillHome: skill.home, projectRoot, dryRun });
