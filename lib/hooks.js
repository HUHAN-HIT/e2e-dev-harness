...
// Normalize a filesystem path for embedding into a hook *command string*.
// Hook commands are executed through a shell (bash, even on Windows). An
// unquoted Windows path like C:\Users\x\scripts is mangled by bash, which
// treats each backslash as an escape (\U -> U, \1 -> 1, ...), collapsing the
// path into garbage that Python then resolves against the cwd. Forward slashes
// are accepted by Python on Windows and survive the shell untouched.
function toShellScriptsDir(scriptsDir) {
  return String(scriptsDir).replace(/\\/g, '/');
}
...
function materializeHooks({ skillHome, projectRoot, dryRun = false, fsImpl = fsDefault }) {
  ...
  // Substitute the shell-safe (forward-slash) form into hook commands; the
  // native scriptsDir is still returned for callers/reporting.
  const template = substituteScriptsDir(
    JSON.parse(fs.readFileSync(templatePath, 'utf8')),
    toShellScriptsDir(scriptsDir)
  );
...
module.exports = { materializeHooks, substituteScriptsDir, mergeHooks, toShellScriptsDir };
