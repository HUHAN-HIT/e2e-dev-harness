...
def ready_handoff_completion_requirements(task: dict | None = None) -> dict:
    ...
    return {
        ...
        "required_steps": [
            ...
            "Run e2e_dev_harness.py hash for declared input/output artifacts that need frontmatter hash lines.",
            "Do not compute or write the final handoff file hash yourself; handoff/dispatch-finish writes it only in the canonical <handoff>.ready.json marker.",
...
def ready_handoff_prompt_lines(task: dict | None = None) -> list[str]:
    ...
    return [
        ...
        "- Do not write the canonical .ready.json marker by hand; handoff --path ... --agent ... or dispatch-finish --handoff ... writes path, sha256, producer_agent, and status: ready after finalizing the handoff.",
...
def manual_worker_packet(repo: Path, schedule_path: Path, state_path: Path | None, task: dict | None, reason: str = "") -> dict:
    ...
    packet = {
        ...
        "next_commands": [
            ...
            "Fresh worker writes only the scheduled outputs from the repair packet: " + (", ".join(outputs) if outputs else "<task outputs>"),
            finish_command,
        ],
