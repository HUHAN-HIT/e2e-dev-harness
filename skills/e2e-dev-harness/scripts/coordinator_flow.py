...
def todo_policy_for_lifecycle(lifecycle: str, state: dict | None = None) -> dict:
    return lifecycle_policy.todo_policy_for_lifecycle(lifecycle, state)
...
def execution_packet_for_lifecycle(
    ...
    packet = {
        ...
        "allowed_writes": list(action.get("allowed_writes", [])),
        ...
        "completion_requires": details["completion_checks"],
...
def evaluate_navigation_state(args) -> tuple[int, dict]:
    ...
    result = {
        ...
        "next": action,
        "preflight": {
            "ready": bool(preflight_result.get("ready", True)),
            "blockers": list(preflight_result.get("blockers", [])),
            "next_single_action": str(preflight_result.get("next_single_action", "")),
        },
