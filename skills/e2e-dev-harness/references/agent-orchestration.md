...
- Before another agent consumes a handoff, fill `agent_id`, `status`, `inputs`, `outputs`, `input_hashes`, `output_hashes`, `consumed_by`, and `open_questions: None`, then run `e2e_dev_harness.py handoff --path <handoff>.md --agent <agent-id>`. That command is the single writer of `<handoff>.ready.json`: it normalizes the frontmatter scalars, atomically renames `<handoff>.md.partial` -> `<handoff>.md`, writes the ready marker, and re-runs `handoff_gate.py`, reporting exact blockers and rolling the marker back if the body is still incomplete. Hashes make stale or silently rewritten artifacts visible at the communication boundary. Do not put the handoff file itself in `outputs` or `output_hashes`; the matching `<handoff>.ready.json` records the handoff file hash. Readers must reject missing ready markers and any leftover partial files.
