...
- Design Reviewer: semantically reviews requirements/use cases before planning; checks AC completeness, affected modules, security-sensitive behavior, and project pattern consistency.
...
- Test Reviewer: semantically reviews red tests before production code; checks happy/failure paths, security cases, contract tests, and shallow "assert only ifCode" style tests.
...
- Implementation Reviewer: semantically reviews code after green/refactor; checks missing artifacts, security flaws, anti-patterns, and divergence from existing project patterns.
...
R1/R2/R3 reviewers must be independent reviewer agents or separate reviewer sessions. The implementation agent, service-scoped Code Developer, or controlling chat may not fill its own semantic review report. Review requests must be assigned to concrete Developer Agent and Reviewer Agent ids before review; placeholders are not valid. Review reports must reference the generated review request, repeat the same Developer Agent and Reviewer Agent values, include a concrete Reviewer Session, reference a Reviewer Invocation JSON audit artifact, include the current SHA-256 Request Hash, declare `Independence: independent-agent`, use a request-scoped context boundary with no inherited developer chat context, and declare `No Code Changes: confirmed`. Same-agent/self-review artifacts block completion.

For multi-service or multi-module runs, every directory under `docs/agent-runs/<run>/service-plans/<service>/` must have service-local R2 and R3 semantic reviews when those phases are required. A global R2/R3 report does not replace service-local reviews because it can reintroduce cross-service context contamination. `reviewer_gate.py` merges explicit `--review-dir` values with inferred `service-plans/*/reviews/` from the same agent run, then blocks missing service-local phases.

Use files as handoff boundaries. Do not rely on chat memory for cross-agent transfer. Keep process files in `docs/agent-runs/`; keep durable human-facing design in `docs/design/`. For multi-service or multi-module changes, keep each service/module implementation plan under `docs/agent-runs/<date-feature>/service-plans/<service>/` so similar service logic does not bleed across agent contexts. Before downstream consumption, handoff artifacts must pass `handoff_gate.py`: concrete `agent_id`, non-draft status, non-empty inputs/outputs, input/output SHA-256 hashes, explicit `consumed_by`, `open_questions: None`, no `*.partial` files, and a matching `<handoff>.ready.json` marker. Writers should produce `<handoff>.md.partial`, compute the final SHA-256, atomically rename it to `<handoff>.md`, then write `<handoff>.ready.json`; readers must not consume files without the ready marker. Read `references/agent-orchestration.md` for mode selection and role contracts, and `references/agent-handoff-schema.md` for the handoff file schema.

When service A depends on service B through HTTP or DMQ, isolation alone is not enough. Freeze the shared contract under `docs/agent-runs/<run>/contracts/<contract-id>.md` before parallel service implementation. Each contract must name producer, consumers, endpoint or topic/tag/group, payload schema, compatibility rule, producer ACK, consumer ACK, contract tests, and status. Run `contract_gate.py` or pass `--contract-dir`; missing ACKs, missing contract tests, draft contracts, or incomplete DMQ topic/tag/group block the workflow.
...
3. R1 design review: run an independent semantic reviewer before planning. Findings become rework or clarification items, not silent assumptions.
4. Plan: design use cases, choose single/multi-agent mode, and write an ExecPlan for complex work.
5. TDD red: invoke or load `superpowers:test-driven-development`, write the first red test, and capture failing evidence.
6. R2 test review: run an independent reviewer on test depth before production code, especially happy/failure paths and security-sensitive cases.
7. TDD green/refactor: implement with Red-Green-Refactor.
8. R3 implementation review: run an independent reviewer on code/test completeness, security, anti-patterns, and project-pattern consistency.
9. Completion gate: prove every acceptance criterion and every required implementation artifact is covered by use cases, service plans, semantic reviews, tests, code refs, business review evidence, and closed rework items.
10. Rework loop: when semantic review, coverage review, tests, business review, or user review finds missed behavior, create a rework item and return to the earliest required phase before more production-code edits.
11. Strict guard and report: run narrow then broadened Maven tests, run `verify --strict-workflow` or `guard`, capture approved memory updates, and report loaded AGENT files, graph status, semantic reviews, tests, commands, coverage, rework status, and residual risks.
...
Planning gate checks clarification readiness, knowledge graph status, and R1 design review. Implementation gate additionally requires red-test evidence and R1+R2 reviews. Completion gate requires `--design-doc`, non-empty red-test evidence, unit-test command evidence, business review evidence, R1+R2+R3 reviews, and a coverage matrix that maps every acceptance criterion to use cases, service/module ownership, tests, code refs, and an accepted status: `implemented`, `covered`, `done`, `pass`, `passed`, or `verified`. Pass `--review-dir` for global and service-local review directories when they are not inferable from the agent-run evidence paths; explicit review dirs are merged with inferred service-local reviews instead of replacing them. If `service-plans/<service>/` exists, required R2/R3 phases must be present for each service. Pass `--handoff-dir` to validate non-reviewer agent handoff files before downstream consumption, and `--contract-dir` to validate frozen HTTP/DMQ contracts before or during cross-service implementation. Review requests must include `Phase`, `Reviewer Role`, `Context Package`, `Forbidden`, `Output`, `Developer Agent`, `Reviewer Agent`, and `Reviewer Invocation`; `Context Package` must be request-scoped with no inherited developer chat context, and `Forbidden` must include self-review and production-code edits. Review reports must include `Phase`, `Reviewer`, `Review Request`, `Developer Agent`, `Reviewer Agent`, `Reviewer Session`, `Reviewer Invocation`, `Request Hash`, `Independence`, `Context Boundary`, `No Code Changes`, `Scope`, `Inputs Reviewed`, `Findings`, `Required Rework`, and `Status`; `blocked`, `open`, `changes-requested`, unsupported statuses, missing review requests, request/report phase mismatch, output mismatch, request hash mismatch, missing or invalid invocation JSON, placeholder IDs, same-agent IDs, non-independent context, or missing service-local review phases block completion. Reviewer Invocation JSON must match the report Developer/Reviewer/Session, point to the same review request and output, declare `fork_context: false`, use a request-only/no-inherited context policy, and be `status: completed`. Semantic review findings must become rework items and are then closed through the rework gate. The coverage gate warns when rows do not explicitly show both happy/success and failure/edge-path coverage; high-risk gaps must become rework items. For multi-module or artifact-heavy designs, completion also requires an implementation manifest at `docs/agent-runs/<run>/evidence/implementation-manifest.md`. The manifest table must include `id`, `module`, `artifact`, `artifact_type`, `source`, `required`, `tests`, `status`, and `evidence`; required rows must point to existing artifacts, real tests, and implemented/verified status. Build it from explicit task requirements, reference implementation patterns, dependency reports, contracts, and service plans before coding. Design artifact extraction is intentionally limited to explicit sections such as `Required Artifacts`, `Affected Classes`, `Required Files`, or `[artifact] ClassName` markers, so historical notes and examples do not become false required artifacts. This catches cases where the coverage matrix only proves self-selected ACs while required files or modules were skipped. If the design is cross-service, completion also requires `--dependency-report`; any unresolved URL/topic/tag/service mapping question blocks completion. If cross-service contracts are declared or `--require-contracts` is used, each contract must have producer/consumer ACKs and contract tests. Unit-test evidence must be structured JSON with `command` and integer `exit_code`; text that merely says PASS is not accepted. The coverage gate extracts acceptance IDs from the design doc; unnumbered acceptance bullets are treated as `AC-1`, `AC-2`, and so on, and every extracted AC must appear in the matrix `id` column. When `--memory-updates` is supplied, completion also blocks unhandled proposed memory updates; mark each one accepted, rejected, deferred, or skipped. Completion also scans inferred or explicit rework items and blocks while any item is `open`, `in-progress`, `blocked`, or `deferred` without `Approval: user-approved`. Completion also runs `spring_static_check.py` by default to catch repository-local constructor-injected types that are not Spring components or `@Bean`s and shared `SimpleDateFormat` fields inside Spring components; use `--skip-spring-static-check` only for non-Spring or exceptional cases.
...
Strict guard blocks missing `prepare`, disabled dependency scan, disabled dependency report writing, skipped Maven, skipped Spring static check during completion, missing clarification status, missing completion gate, failed completion gate, missing independent semantic review evidence, failed Maven, and unresolved dependency questions. A skip can pass only with an explicit approval file containing `Approval: user-approved`.
...
python skills/e2e-dev-workflow/scripts/reviewer_gate.py . --review-dir docs/agent-runs/<run>/reviews --require-phase design --require-phase test --require-phase implementation
python skills/e2e-dev-workflow/scripts/handoff_gate.py . --handoff-dir docs/agent-runs/<run>/handoffs
python skills/e2e-dev-workflow/scripts/contract_gate.py . --contract-dir docs/agent-runs/<run>/contracts
