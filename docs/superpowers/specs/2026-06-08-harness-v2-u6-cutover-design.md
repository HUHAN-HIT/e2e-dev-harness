# U6 — M5 Cutover Design (v2 default + migration docs + delete legacy)

**Date:** 2026-06-08
**Unit:** U6 (final unit of harness-v2-m2)
**Status:** design (pre-writing-plans)
**SSOT baseline:** `2026-06-07-harness-v2-redesign-design.md` §1, §5, §6, §14 (M5), §16
...
Retire the legacy `skills/e2e-dev-harness/` and make v2 the sole, default harness — **only after** proving no capability loss (design §14 M5). Outward-facing: Node + Python entry points switch to v2; version 0.1.0 → 0.2.0; migration doc published; legacy deleted in a dedicated, reversible-by-revert commit.
...
| Legacy capability | v2 coverage | Verdict |
|---|---|---|
| 35-verb CLI (start/next/dispatch/…/doctor/preflight/map/timeline) | 6 verbs + `validate-pipeline` (§6 mapping table) | **covered** |
| `phase_guard.py` PreToolUse (phase-lock code writes) | `phase_guard_v2.py` (U7) | **covered (U7)** |
| `harness_stop_guard.py` Stop (continue-until-verified) | `stop_guard_v2.py` (U7) | **covered (U7)** |
...
| legacy state aliases / `worker_running_unverified` shim | single v2 state enum | **dropped (§5)** |
...
- v2 vendors its own `_legacy/` leaves (kg/scanner/memory) — self-contained; no runtime path into `skills/e2e-dev-harness/`.
- Full v2 suite green (223 passed) with legacy present **and** must stay green after delete (Stage 5 gate).
...
|---|---|---|
| `package.json` | `version: 0.2.0`, `bin → bin/e2e-harness.js`, `files` includes `skills/e2e-dev-harness-v2` (not legacy) | validate + commit (Stage 2a) |
| `lib/resolve.js` | `V2_VERBS` → dispatches to `e2e_dev_harness_v2.py` | validate + commit (Stage 2a) |
...
| **`pyproject.toml`** | `[project.scripts]` → **`e2e_dev_harness:main` (LEGACY)**, `package-dir` = legacy scripts | **switch to v2 (Stage 2b)** |
| `skills/e2e-dev-harness-v2/SKILL.md` | exists, not marked default | mark default (Stage 2c) |
| `skills/e2e-dev-harness/` (legacy) | present, frozen | delete (Stage 5) |

**Critical ordering invariant:** `pyproject.toml` currently *points at* legacy. The Python entry MUST switch to v2 (Stage 2b) **before** legacy is deleted (Stage 5), or `pip install` / console-scripts break. The plan enforces Stage 2b ⟶ Stage 5 ordering.
...
2. **pyproject → v2 before delete.** Hard ordering constraint (§3). pyproject `[project.scripts]` retargets to the v2 entry; `package-dir`/`packages` repoint to `skills/e2e-dev-harness-v2/scripts` (`harness_v2*` + the `e2e_dev_harness_v2` module). Keep the three legacy alias names? → **No**: rename console scripts to the v2 surface (`e2e-harness-v2` matching the argparse `prog`), with `e2e-dev-harness`/`e2eh` retained as aliases pointing to v2 for muscle-memory continuity. (Final alias set resolved in the plan after reading the v2 entry's `main` signature.)
...
4. **Installer wires U7 hooks (Stage 3).** The Node + Python installers register `phase_guard_v2`/`stop_guard_v2` (claude settings + opencode plugin) from `skills/e2e-dev-harness-v2/hooks/*.example.*`, rewriting `__HARNESS_V2_SCRIPTS__` to the installed abs path. Replaces the legacy `install_hooks.py` phase_guard/stop_guard wiring — **no capability gap** (U7 supplied the v2 hooks).
5. **Default = v2.** v2 `SKILL.md` description/title marked as the canonical harness; the user-global `CLAUDE.md`/project pointer (if any) and installer manifest point to v2. Legacy SKILL.md removed with the rest in Stage 5.
...
| **2b** | pyproject `[project.scripts]` + `package-dir`/`packages` → v2; `python -m build`/`pip install -e .` smoke; console-script smoke | feat commit | yes (revert) |
| **2c** | v2 `SKILL.md` set as default canonical harness | feat commit | yes |
| **3** | installer registers U7 v2 hooks (Node + Python paths) + tests | feat commit | yes |
...
**Verification gates:**
- After 2a/2b: respective entry smoke passes (Node bin resolves a v2 verb; Python console script runs `start`).
- After 3: installer tests green (hook files land at install target with rewritten path).
- After 5: `python -m pytest skills/e2e-dev-harness-v2/tests -q` = 223 passed; `grep -r 'e2e-dev-harness/scripts\|e2e_dev_harness\b'` over repo (excluding docs/git history) returns no *runtime* reference.
...
- `skills/e2e-dev-harness-v2/SKILL.md` (2c)
...
- **R1 — pip entry breaks if delete precedes pyproject switch.** Mitigated by the Stage 2b⟶5 ordering invariant (§3) baked into the plan.
...
## 9. Next step
`superpowers:writing-plans` → staged TDD plan honoring the Stage 2b⟶5 ordering invariant, then inline execution with per-stage commits.
