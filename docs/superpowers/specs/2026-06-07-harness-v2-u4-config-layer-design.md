# Harness v2 — U4: M3 Config Layer (pipelines-as-config + `validate-pipeline`) — Design

- **Status:** Approved — ready for `superpowers:writing-plans`
- **Date:** 2026-06-07
- **Branch:** `harness-v2-m2`
- **Unit:** U4 of [harness-v2 remaining-work roadmap](../plans/2026-06-07-harness-v2-remaining-work-roadmap.md)
- **Design source:** [harness-v2 redesign](2026-06-07-harness-v2-redesign-design.md) §11 (adaptive pruning), §12 (pipelines-as-config), §15 (invariants I1/I2), §16 (YAGNI).
...
`scripts/harness_v2/pipeline.py`. U4:

1. Moves them into shipped declarative `pipelines/*.yaml` interpreted by the state machine —
   built-in tiers become factory config with no special privilege.
...
Built-in files: `skills/e2e-dev-harness-v2/pipelines/{minimal,standard,critical,audited}.yaml`.
...
`e2e-harness-v2 validate-pipeline --pipeline <name-or-path>`:
...
**New:**
- `skills/e2e-dev-harness-v2/pipelines/{minimal,standard,critical,audited}.yaml`
- `skills/e2e-dev-harness-v2/scripts/harness_v2/core/pipeline_validate.py`
- `skills/e2e-dev-harness-v2/scripts/harness_v2/cli/commands/validate_pipeline.py`
- 4 test files (§8).

**Edited (v2-internal only — no legacy edits per §15):**
- `scripts/harness_v2/pipeline.py` — constants → loader/interpreter + `spine_for_state`.
- `scripts/harness_v2/core/run_state.py` — optional `pipeline_spec` field.
- `scripts/harness_v2/cli/main.py` — register `validate-pipeline`; add `start --pipeline`.
- `scripts/harness_v2/cli/commands/start.py` — pipeline resolution + validation guard + embed.
- `scripts/harness_v2/cli/commands/{next,dispatch,gate,status}.py` — use `spine_for_state`.
...
- Per CLAUDE.md, run `gitnexus_impact` before editing existing v2 symbols (`pipeline.build_spine`,
  ...
  tool warns.
- No legacy edits before M5 (§15): all changes are new v2 files or v2-internal edits.
