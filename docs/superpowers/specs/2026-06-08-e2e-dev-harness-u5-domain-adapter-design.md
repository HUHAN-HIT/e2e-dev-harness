# Harness e2e-dev-harness �?U5 DomainAdapter seam (M4 frontend) �?Design Spec

- **Status:** Approved (brainstorming) �?pending writing-plans
- **Date:** 2026-06-08
- **Branch:** `e2e-dev-harness-m2`
- **Roadmap unit:** U5 (`docs/superpowers/plans/2026-06-07-e2e-dev-harness-remaining-work-roadmap.md` §U5)
- **Design source:** `2026-06-07-e2e-dev-harness-redesign-design.md` §13 (领域适配 seam), §12 (流水线即配置), §15 (invariants)
...
- The **backend e2e simulates workers**: it writes real artifact files and submits their paths, asserting `start→VERIFIED` terminates in �? steps (and that fake paths never reach `VERIFIED`). No real test suite runs.
...
## 4. The "核心零改�? boundary (stated precisely)

§13 says "核心零改�?" To keep that claim honest, the spec defines exactly what is and isn't touched:
...
Fixture: `tests/fixtures/frontend_app/` �?`package.json` (with a frontend dep), one component under `src/`, and a vitest test file **as data** (never executed). Existing full e2e-dev-harness suite must stay green (parity).
