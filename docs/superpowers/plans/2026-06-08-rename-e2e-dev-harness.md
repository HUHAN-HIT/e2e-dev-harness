# Rename e2e-dev-harness-v2 → e2e-dev-harness Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Promote the `e2e-dev-harness-v2` skill to its canonical name `e2e-dev-harness` across the entire repo — skill directory, internal Python package, entry/hook scripts, Node installer/CLI, worker skills, both test suites, and all docs (living + historical).

**Architecture:** This is a mechanical multi-layer rename, not a behavior change. There are **no code-level `_v2` symbols** (verified: zero `def *_v2` / `class *V2`), so the work is `git mv` (preserve history) + ordered token substitution + dual-suite verification. The existing Python (`pytest`) and Node (`node --test`) suites are the red/green oracle: every phase ends green, or it is not done.
...
|---|---|---|
| `skills/e2e-dev-harness-v2/` (dir) | `skills/e2e-dev-harness/` | `git mv` |
| `e2e_dev_harness_v2.py` (entry) | `e2e_dev_harness.py` | `git mv` |
| `harness_v2/` (py package) | `e2e_harness/` | `git mv`; package renamed to avoid PyPI/global namespace collision |
| `phase_guard_v2.py` | `phase_guard.py` | `git mv` |
| `stop_guard_v2.py` | `stop_guard.py` | `git mv` |
| `e2e_dev_harness_v2` (token) | `e2e_dev_harness` | replace **before** `harness_v2` |
| `e2e-dev-harness-v2` (kebab) | `e2e-dev-harness` | replace **before** `harness-v2` |
| `harness_v2` (token/import) | `e2e_harness` | |
| `phase_guard_v2` / `stop_guard_v2` (token) | `phase_guard` / `stop_guard` | |
| `__HARNESS_V2_SCRIPTS__` | `__HARNESS_SCRIPTS__` | Node placeholder |
| console script `e2e-harness-v2` | **deleted** | keep `e2e-dev-harness`/`e2eh`; never add bare `e2e-harness` (collides with Node bin) |
| SKILL title "E2E Dev Harness v2" | "E2E Dev Harness" | |

**Token replacement order (CRITICAL — specific before general):**
1. `e2e_dev_harness_v2` → `e2e_dev_harness`
2. `e2e-dev-harness-v2` → `e2e-dev-harness`
3. `phase_guard_v2` → `phase_guard`
4. `stop_guard_v2` → `stop_guard`
5. `__HARNESS_V2_SCRIPTS__` → `__HARNESS_SCRIPTS__`
6. `harness_v2` → `e2e_harness`
7. (docs only, manual) bare `harness-v2` product references → `e2e-dev-harness`
...
**Files:**
- Move: `skills/e2e-dev-harness-v2/` → `skills/e2e-dev-harness/`
- Move: `…/scripts/e2e_dev_harness_v2.py` → `…/scripts/e2e_dev_harness.py`
- Move: `…/scripts/harness_v2/` → `…/scripts/e2e_harness/`
- Move: `…/scripts/e2e_harness/adapters/hooks/phase_guard_v2.py` → `…/phase_guard.py`
- Move: `…/scripts/e2e_harness/adapters/hooks/stop_guard_v2.py` → `…/stop_guard.py`

- [ ] **Step 1: Rename the skill directory**

```bash
git mv skills/e2e-dev-harness-v2 skills/e2e-dev-harness
```

- [ ] **Step 2: Rename the entry script and Python package**

```bash
git mv skills/e2e-dev-harness/scripts/e2e_dev_harness_v2.py skills/e2e-dev-harness/scripts/e2e_dev_harness.py
git mv skills/e2e-dev-harness/scripts/harness_v2 skills/e2e-dev-harness/scripts/e2e_harness
```

- [ ] **Step 3: Rename the two hook scripts**

```bash
git mv skills/e2e-dev-harness/scripts/e2e_harness/adapters/hooks/phase_guard_v2.py skills/e2e-dev-harness/scripts/e2e_harness/adapters/hooks/phase_guard.py
git mv skills/e2e-dev-harness/scripts/e2e_harness/adapters/hooks/stop_guard_v2.py skills/e2e-dev-harness/scripts/e2e_harness/adapters/hooks/stop_guard.py
```
...
```
Expected: `e2e_dev_harness.py` + `e2e_harness/` present; hooks dir shows `phase_guard.py` and `stop_guard.py` (no `_v2`). Imports are now broken — that is expected; Task 2 fixes them.

- [ ] **Step 5: Commit the moves**

```bash
git add -A
git commit -m "refactor(rename): git mv e2e-dev-harness-v2 skill, harness_v2 pkg, _v2 hooks to canonical names"
```
...
**Files:**
- Modify: every `.py` under `skills/e2e-dev-harness/scripts/` and `skills/e2e-dev-harness/tests/` referencing old tokens (≈59 import files; 0 code symbols).

- [ ] **Step 1: Apply ordered token substitution inside the skill tree**

```bash
cd skills/e2e-dev-harness
files=$(git grep -lIE "e2e_dev_harness_v2|harness_v2|phase_guard_v2|stop_guard_v2" -- . )
for f in $files; do
  sed -i \
    -e 's/e2e_dev_harness_v2/e2e_dev_harness/g' \
    -e 's/phase_guard_v2/phase_guard/g' \
    -e 's/stop_guard_v2/stop_guard/g' \
    -e 's/harness_v2/e2e_harness/g' "$f"
done
cd ../..
```
...
Run:
```bash
git grep -nI "harness_v2\|e2e_dev_harness_v2\|phase_guard_v2\|stop_guard_v2" -- skills/e2e-dev-harness/scripts skills/e2e-dev-harness/tests
```
Expected: **no output**.
...
```bash
git add -A
git commit -m "refactor(rename): repoint Python imports harness_v2->e2e_harness, drop _v2 in skill"
```
...
- Frontmatter line 2: `name: e2e-dev-harness-v2` → `name: e2e-dev-harness`
...
- Title line 6: `# E2E Dev Harness v2` → `# E2E Dev Harness`
- Body command block (`S=...`): `skills/e2e-dev-harness-v2/scripts/e2e_dev_harness_v2.py` → `skills/e2e-dev-harness/scripts/e2e_dev_harness.py`

- [ ] **Step 2: Sweep any remaining tokens in this file**

```bash
sed -i -e 's/e2e_dev_harness_v2/e2e_dev_harness/g' -e 's/e2e-dev-harness-v2/e2e-dev-harness/g' -e 's/harness_v2/e2e_harness/g' skills/e2e-dev-harness/SKILL.md
git grep -nI "v2\|harness_v2\|e2e-dev-harness-v2" -- skills/e2e-dev-harness/SKILL.md
```
Expected: no `harness_v2` / `e2e-dev-harness-v2` / `e2e_dev_harness_v2`. (A stray "v2" only acceptable if part of prose you intentionally keep — there should be none.)
...
git commit -m "refactor(rename): SKILL.md identity e2e-dev-harness-v2 -> e2e-dev-harness"
```
...
- `[project.scripts]` — set to exactly:
  ...
  ```
  (Delete the `e2e-harness-v2 = …` line. Do NOT add a bare `e2e-harness` script — it collides with the Node CLI on PATH.)
- `package-dir`: `{ "" = "skills/e2e-dev-harness-v2/scripts" }` → `{ "" = "skills/e2e-dev-harness/scripts" }`
- `packages = { find = { where = ["skills/e2e-dev-harness-v2/scripts"], include = ["harness_v2*"] } }` → `where = ["skills/e2e-dev-harness/scripts"]`, `include = ["e2e_harness*"]`

- [ ] **Step 2: Guard grep**

```bash
git grep -nI "harness_v2\|e2e-dev-harness-v2\|e2e-harness-v2" -- pyproject.toml
```
Expected: no output.
...
git commit -m "refactor(rename): pyproject pkg path/discovery + drop e2e-harness-v2 script"
```
...
for f in skills/e2e-harness-*/SKILL.md; do
  sed -i \
    -e 's/e2e_dev_harness_v2/e2e_dev_harness/g' \
    -e 's/e2e-dev-harness-v2/e2e-dev-harness/g' \
    -e 's/(e2e-dev-harness-v2)/(e2e-dev-harness)/g' \
    -e 's/## v2 契约/## 契约/g' \
    -e 's/harness_v2/e2e_harness/g' "$f"
done
...
```bash
git grep -nI "e2e-dev-harness-v2\|e2e_dev_harness_v2\|harness_v2" -- skills/e2e-harness-*/SKILL.md
```
Expected: no output.
...
for f in lib/paths.js lib/install.js lib/lifecycle.js lib/resolve.js lib/init.js lib/hooks.js bin/e2e-harness.js tools/install-e2e-dev-harness.mjs tools/clean-pack.mjs tools/pre-merge-check.mjs package.json; do
  sed -i \
    ...
    -e 's/phase_guard_v2/phase_guard/g' \
    -e 's/stop_guard_v2/stop_guard/g' \
    -e 's/__HARNESS_V2_SCRIPTS__/__HARNESS_SCRIPTS__/g' \
    -e 's/harness_v2/e2e_harness/g' "$f"
done
```
...
```bash
git grep -nI "harness_v2\|e2e-dev-harness-v2\|e2e_dev_harness_v2\|HARNESS_V2\|phase_guard_v2\|stop_guard_v2" -- lib/ bin/ tools/ package.json
```
Expected: no output.
...
```bash
git grep -nI "harness_v2\|e2e-dev-harness-v2\|e2e_dev_harness_v2\|HARNESS_V2\|phase_guard_v2\|stop_guard_v2" -- test/ tests/
```
Expected: no output.
...
for f in README.md MIGRATION.md CHANGELOG.md; do
  sed -i \
    ...
    -e 's/phase_guard_v2/phase_guard/g' \
    -e 's/stop_guard_v2/stop_guard/g' \
    -e 's/harness_v2/e2e_harness/g' \
    -e 's/`e2e-harness-v2`[^`]*//g' "$f"
done
```
...
- `CHANGELOG.md`: add a new top entry:
  `**Rename: e2e-dev-harness-v2 promoted to canonical e2e-dev-harness; Python package harness_v2 → e2e_harness; entry/hook scripts drop _v2; console script e2e-harness-v2 removed (e2e-dev-harness/e2eh retained).**`
...
```bash
git grep -nI "harness_v2\|e2e-dev-harness-v2\|e2e_dev_harness_v2\|e2e-harness-v2" -- README.md MIGRATION.md CHANGELOG.md
```
Expected: no output.
...
**Files:**
- Modify (content only): `docs/superpowers/specs/*harness-v2*.md`, `docs/superpowers/plans/*harness-v2*.md`, `docs/superpowers/HANDOFF-harness-v2.md`, and `docs/*harness*.md` that reference old tokens.

> Decision baked in: rewrite the **product/code tokens** inside historical docs, but **keep dated filenames intact** (e.g. `2026-06-07-harness-v2-redesign-design.md` stays — renaming it would break cross-links and git history of those records). Bare milestone tokens like `harness-v2-m2` (branch/milestone labels) are left as-is unless they are clearly the product name in prose.

- [ ] **Step 1: Substitute code/product tokens across historical docs**

```bash
files=$(git grep -lI "e2e-dev-harness-v2\|e2e_dev_harness_v2\|harness_v2\|phase_guard_v2\|stop_guard_v2" -- docs/)
for f in $files; do
  sed -i \
    -e 's/e2e_dev_harness_v2/e2e_dev_harness/g' \
    -e 's/e2e-dev-harness-v2/e2e-dev-harness/g' \
...
- [ ] **Step 2: Guard grep (code/product tokens cleared; bare `harness-v2` filename refs may remain by design)**

```bash
git grep -nI "harness_v2\|e2e-dev-harness-v2\|e2e_dev_harness_v2\|phase_guard_v2\|stop_guard_v2" -- docs/
```
Expected: no output. (Then review remaining `harness-v2` hits with `git grep -nI "harness-v2" -- docs/` and confirm each is a dated filename/milestone label intentionally kept.)
...
Run:
```bash
git grep -nI "harness_v2\|e2e-dev-harness-v2\|e2e_dev_harness_v2\|phase_guard_v2\|stop_guard_v2\|__HARNESS_V2_SCRIPTS__\|e2e-harness-v2"
```
...
3. **`sed` ordering already encoded.** Every batch lists `e2e_dev_harness_v2`/`e2e-dev-harness-v2` substitutions before `harness_v2` to avoid corrupting the longer tokens (they contain `harness_v2`/`harness-v2` as substrings).
...
5. **No run-state migration needed.** Verified: `harness_v2` is not used as a persisted schema/version string in run-state, so existing/new run-states are unaffected by the package rename.
