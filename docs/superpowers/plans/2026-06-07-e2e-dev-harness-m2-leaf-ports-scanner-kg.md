# Harness e2e-dev-harness �?M2 Leaf Ports: scanner + kg-evidence Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Port the two clean, test-backed leaves deferred from M2 �?`kg-evidence` (`kg_refresh`) and `scanner` (`cross_service_dependency_scan` + scanners facade) �?into `skills/e2e-dev-harness/` behind narrow interfaces, bringing their legacy tests green, with zero logic edits (design §5 "作为�?port,逻辑不动,只包一层窄接口").

**Architecture:** Each leaf is vendored verbatim into a self-contained flat module dir `e2e_harness/adapters/<leaf>/_legacy/` (keeping the legacy flat imports working unchanged), exposed to e2e-dev-harness callers through a thin `__init__.py` shim that inserts that dir on `sys.path` once and re-exports the public functions. The legacy tests are copied verbatim into the e2e-dev-harness test dir, changing **only** the path constant that points at the vendored modules. The two leaves are independent and separately committable �?kg lands first (small, clean), scanner second (heavier: 1055L scan module + 291L plugin_registry + 674L test).

**Tech Stack:** Python 3.13, pytest + stdlib `unittest`, existing e2e-dev-harness `tests/conftest.py` sys.path convention.
...
**Files:**
- Create dir: `skills/e2e-dev-harness/scripts/e2e_harness/adapters/kg/_legacy/`
...
mkdir -p skills/e2e-dev-harness/scripts/e2e_harness/adapters/kg/_legacy
cp skills/e2e-dev-harness/scripts/common.py     skills/e2e-dev-harness/scripts/e2e_harness/adapters/kg/_legacy/common.py
cp skills/e2e-dev-harness/scripts/kg_refresh.py skills/e2e-dev-harness/scripts/e2e_harness/adapters/kg/_legacy/kg_refresh.py
```
...
Run:
```bash
diff skills/e2e-dev-harness/scripts/common.py     skills/e2e-dev-harness/scripts/e2e_harness/adapters/kg/_legacy/common.py
diff skills/e2e-dev-harness/scripts/kg_refresh.py skills/e2e-dev-harness/scripts/e2e_harness/adapters/kg/_legacy/kg_refresh.py
```
...
**Files:**
- Create: `skills/e2e-dev-harness/scripts/e2e_harness/adapters/kg/__init__.py`
...
python -c "from e2e_harness.adapters.kg import detect, choose_tools, suggested_commands; print('ok', detect.__name__, choose_tools.__name__)"
```
...
**Files:**
- Create: `skills/e2e-dev-harness/tests/test_kg_refresh.py` (copy of `tests/test_kg_refresh.py`)

- [ ] **Step 1: Copy the legacy test, then change only its path constant**

Copy `tests/test_kg_refresh.py` to `skills/e2e-dev-harness/tests/test_kg_refresh.py` byte-for-byte, then replace lines 16�?9. The legacy header is:
...
Replace those four lines with (point at the vendored leaf; `parents[1]` is the e2e-dev-harness skill root):
...
Run: `cd skills/e2e-dev-harness && python -m pytest tests/test_kg_refresh.py -v`
...
- [ ] **Step 3: Run the full e2e-dev-harness suite (no regressions)**
...
```bash
git add skills/e2e-dev-harness/scripts/e2e_harness/adapters/kg/ skills/e2e-dev-harness/tests/test_kg_refresh.py
git commit -m "feat(e2e-dev-harness): port kg-evidence leaf (kg_refresh) behind narrow adapter + its test"
```
...
- Create dir: `skills/e2e-dev-harness/scripts/e2e_harness/adapters/scanner/_legacy/`
...
```bash
cd "C:/Users/14907/Documents/Codex/2026-05-23/skill-skill-superpowers-skill-tdd-graphify"
DST=skills/e2e-dev-harness/scripts/e2e_harness/adapters/scanner/_legacy
...
**Files:**
- Create: `skills/e2e-dev-harness/scripts/e2e_harness/adapters/scanner/__init__.py`
...
```bash
cd skills/e2e-dev-harness
python -c "from e2e_harness.adapters.scanner import discover_scope; from pathlib import Path; print(discover_scope(Path('.'))['scanner'])"
```
...
**Files:**
- Create: `skills/e2e-dev-harness/tests/test_scanner.py` (copy of `tests/test_scanner.py`)

- [ ] **Step 1: Copy the legacy test, then change only its path constant**

Copy `tests/test_scanner.py` to `skills/e2e-dev-harness/tests/test_scanner.py` byte-for-byte. The legacy header (lines 16�?9) is:
...
Run: `cd skills/e2e-dev-harness && python -m pytest tests/test_scanner.py -v`
...
python -m pytest skills/e2e-dev-harness/tests/test_scanner.py -q   # ported
```
...
**Files:**
- Create: `skills/e2e-dev-harness/tests/test_scanner_ast.py` (copy of `tests/test_scanner_ast.py`)

This test imports the module differently �?`import cross_service_dependency_scan as scan_mod` at module top with **no** `sys.path` block �?and skips the whole module unless `tree_sitter` + `tree_sitter_java` are installed (legacy lines 12�?4).

- [ ] **Step 1: Copy the test and add the path bootstrap**

Copy `tests/test_scanner_ast.py` to `skills/e2e-dev-harness/tests/test_scanner_ast.py` byte-for-byte. The legacy top (lines 1�?0) is:
...
It relies on the legacy `conftest`/`sys.path` to resolve `cross_service_dependency_scan`. In e2e-dev-harness, insert the vendored dir **before** that import. Replace the block above with:
...
Run: `cd skills/e2e-dev-harness && python -m pytest tests/test_scanner_ast.py -v`
...
- [ ] **Step 3: Full e2e-dev-harness suite green**
...
```bash
git add skills/e2e-dev-harness/scripts/e2e_harness/adapters/scanner/ \
        skills/e2e-dev-harness/tests/test_scanner.py \
        skills/e2e-dev-harness/tests/test_scanner_ast.py
git commit -m "feat(e2e-dev-harness): port scanner leaf (cross_service scan + facade) behind narrow adapter + its tests"
```
...
Expected: only **new** files under `skills/e2e-dev-harness/scripts/e2e_harness/adapters/{kg,scanner}/` and `skills/e2e-dev-harness/tests/test_{kg_refresh,scanner,scanner_ast}.py`. **No** edits to any legacy `skills/e2e-dev-harness/` file (M5 owns those). The pre-existing untracked dirs (`docs/agent-runs/nonexistent/`, `skills/e2e-dev-harness/scripts/.e2e/`, `snapshots/`) are test scratch and unrelated.

Run: `npx gitnexus detect-changes` (or `gitnexus_detect_changes` MCP) and confirm only the two new e2e-dev-harness adapter packages + their tests appear.

---

## Exit checklist (this plan done?)

- [ ] kg-evidence: `kg_refresh` vendored verbatim behind `e2e_harness.adapters.kg`; `test_kg_refresh.py` ported and green.
- [ ] scanner: `cross_service_dependency_scan` + `plugin_registry` + scanners facade vendored verbatim behind `e2e_harness.adapters.scanner`; `test_scanner.py` ported & green; `test_scanner_ast.py` ported (skips cleanly without tree_sitter).
- [ ] All vendored module bodies are **byte-identical** to legacy (verified by `diff`) �?logic unchanged per design §5.
- [ ] Full e2e-dev-harness suite green; only new e2e-dev-harness files changed; no legacy edits.

## Deferred (NOT in this plan)

- **Wire scanner �?tier escalation.** `adapters/tier/classify.py` notes multi-service/dependency-report escalation is "intentionally omitted until the scanner leaf is ported." Now unblocked, but it's a **behavior change to a e2e-dev-harness leaf** (classification escalates on scanner output), not a port �?give it its own small TDD task/plan with golden-fixture coverage so the escalation thresholds are asserted, not silently introduced.
