# e2e-dev-harness run-state Concurrency Hardening Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Make the e2e-dev-harness single-file SSOT (`run-state.json`) safe under concurrent writers so parallel reviewer evidence (r1/r2/r3) can never be silently lost.

**Architecture:** e2e-dev-harness already eliminated v1's multi-file split-brain �?`run-state.json` is the sole SSOT and all derived views (navigation, gates, phase advance) are recomputed, not stored. The remaining gap is *intra-file* concurrency: every mutating CLI verb does `load() �?mutate �?save()` with no lock, so concurrent verbs lose updates (last-`os.replace`-wins). We add one exclusive-lock `mutate()` helper in `run_state.py` and route the three concurrent-capable verbs (`submit`, `dispatch`, `next`) through it. `gate` is read-only (excluded); `start` is run-creation / single-writer (excluded).
...
`git status` shows a **staged rename** `skills/e2e-dev-harness/ �?skills/e2e-dev-harness/`. All paths below use the **post-rename live path** `skills/e2e-dev-harness/...`. If a file fails to open (this tree is under a cloud-synced `Documents` folder and intermittently returns "file not found" for materialized-but-not-downloaded files), re-run the read/test once to force materialization, or `git show HEAD:skills/e2e-dev-harness/<path>` to read the committed copy.

## File Structure

- **Modify** `skills/e2e-dev-harness/scripts/e2e_harness/core/run_state.py` �?add `_lock()` + `mutate()`, make `save()` tmp name unique. (Currently 61 lines; one clear responsibility = SSOT persistence. Adding the lock here keeps the critical section co-located with `load`/`save`.)
- **Modify** `skills/e2e-dev-harness/scripts/e2e_harness/cli/commands/submit.py` �?route through `mutate()`.
- **Modify** `skills/e2e-dev-harness/scripts/e2e_harness/cli/commands/dispatch.py` �?route the state write through `mutate()`.
- **Modify** `skills/e2e-dev-harness/scripts/e2e_harness/cli/commands/next.py` �?route the evaluate+save through `mutate()`.
- **Test** `skills/e2e-dev-harness/tests/test_run_state.py` �?concurrency unit tests.
- **Test (create)** `skills/e2e-dev-harness/tests/test_concurrency_e2e-dev-harness.py` �?parallel-reviewer integration test.

**Test command (used throughout):**
```bash
python -m pytest skills/e2e-dev-harness/tests/test_run_state.py skills/e2e-dev-harness/tests/test_concurrency_e2e-dev-harness.py -v
```
(`tests/conftest.py` puts `scripts/` on `sys.path`, so `from e2e_harness.core import run_state` works.)
...
Expected: PASS (all existing tests). This also forces the e2e-dev-harness source files to materialize on disk and confirms the import path works before any edit.

- [ ] **Step 2: Capture baseline**

Run: `git stash list && git status --short | head`
Expected: a clean view of the staged rename; no unexpected modifications under `e2e_harness/`.
...
Expected: FAIL �?`AttributeError: module 'e2e_harness.core.run_state' has no attribute 'mutate'`.
...
git commit -m "fix(e2e-dev-harness): add locked run_state.mutate() to prevent lost updates"
```
...
- Test: `skills/e2e-dev-harness/tests/test_concurrency_e2e-dev-harness.py` (create)

- [ ] **Step 1: Write the failing integration test**

Create `skills/e2e-dev-harness/tests/test_concurrency_e2e-dev-harness.py`:
...
Run: `python -m pytest skills/e2e-dev-harness/tests/test_concurrency_e2e-dev-harness.py -v --count=1`
...
(Keep the existing imports `from e2e_harness.core import run_state, engine` and `from pathlib import Path`.)

- [ ] **Step 4: Run to verify it passes deterministically**

Run: `python -m pytest skills/e2e-dev-harness/tests/test_concurrency_e2e-dev-harness.py skills/e2e-dev-harness/tests/test_submit.py -v`
Expected: PASS �?all three reviewer keys survive every run; existing submit tests still green.

- [ ] **Step 5: Commit**

```bash
git add skills/e2e-dev-harness/scripts/e2e_harness/cli/commands/submit.py skills/e2e-dev-harness/tests/test_concurrency_e2e-dev-harness.py
git commit -m "fix(e2e-dev-harness): route submit through locked mutate (parallel review safe)"
```
...
import types
from pathlib import Path
from e2e_harness.core import run_state as _rs
from e2e_harness.cli.commands import dispatch as _dispatch_cmd
...
(Keep existing imports: `from e2e_harness.core import run_state, dispatch`, `from e2e_harness import pipeline`, `from e2e_harness.adapters import runtime`.)
...
```bash
git add skills/e2e-dev-harness/scripts/e2e_harness/cli/commands/dispatch.py skills/e2e-dev-harness/tests/test_dispatch.py
git commit -m "fix(e2e-dev-harness): route dispatch state write through locked mutate"
```
...
```python
from e2e_harness.cli.commands import next as _next_cmd
...
(Keep existing imports: `from pathlib import Path`, `from e2e_harness.core import run_state, engine, navigation`, `from e2e_harness import pipeline`.)

- [ ] **Step 4: Run the full suite**

Run: `python -m pytest skills/e2e-dev-harness/tests/ -v`
Expected: PASS (whole e2e-dev-harness suite green).

- [ ] **Step 5: Commit**

```bash
git add skills/e2e-dev-harness/scripts/e2e_harness/cli/commands/next.py skills/e2e-dev-harness/tests/test_dispatch.py
git commit -m "fix(e2e-dev-harness): route next evaluate/save through locked mutate"
```

---

## Task 5 (OPTIONAL �?requires user decision): pin `current_phase` lag semantics (Finding e2e-dev-harness)

> **Decision gate:** Finding e2e-dev-harness is a *semantic* choice, not a safety bug. Today `current_phase` advances **only** on `next` (lazy); `submit` records evidence but does not advance. This is **fail-safe** (the phase guard stays restrictive until `next` runs). Do **not** implement this task until the user confirms the desired semantics:
...
**Files (Option A only):**
- Modify: `skills/e2e-dev-harness/scripts/e2e_harness/core/run_state.py` (docstring on `save`/module) and/or `lifecycle.py` (Phase docstring) to state: *"`current_phase` = last phase evaluated by `next`; it may lag the true gate-derived phase between a `submit` and the next `next`. Readers (e.g. phase guard) treat this as fail-safe."*
- Test: `skills/e2e-dev-harness/tests/test_concurrency_e2e-dev-harness.py`

- [ ] **Step 1: Add the pin test**

```python
from e2e_harness.cli.commands import submit as _submit_cmd
...
Run: `python -m pytest skills/e2e-dev-harness/tests/test_concurrency_e2e-dev-harness.py::test_submit_does_not_advance_current_phase -v`
Expected: PASS.

- [ ] **Step 3: Commit**

```bash
git add skills/e2e-dev-harness/scripts/e2e_harness/core/run_state.py skills/e2e-dev-harness/tests/test_concurrency_e2e-dev-harness.py
git commit -m "docs(e2e-dev-harness): pin current_phase lazy-advance fail-safe semantics"
```
...
- **Finding V4 (schema migration):** `run_state.load()` hard-fails on schema mismatch. This is correct fail-closed behavior today and there is only one schema (`...run-state.v1`). YAGNI until a `.e2e-dev-harness` schema actually ships; revisit then with a migration hook.
...
- **Spec coverage:** V1 (lost updates) �?Tasks 1�?. V3 (shared `.tmp`) �?Task 1 Step 3 (process-unique tmp). e2e-dev-harness (current_phase lag) �?Task 5 (decision-gated). V4 �?explicitly deferred. `.phase-lock` / multi-file split-brain �?already eliminated in e2e-dev-harness, no task needed. �?- **Type/name consistency:** `run_state.mutate(path, fn, now=None)` signature is identical across Tasks 1�?. `DispatchStatus.DISPATCHED.value == "dispatched"` matches the dispatch test assertion. `engine.submit_evidence`, `engine.evaluate`, `navigation.navigation_map`, `pipeline.spine_for_state` signatures unchanged from current source. �?- **No placeholders:** every code step shows full code; every run step shows command + expected result. �?- **Concurrency test validity:** Task 1's 20-thread test and Task 2's 3-reviewer test fail without the lock (lost keys) and pass with it (all keys survive) �?they exercise the real defect, not a proxy. �?